...
class Foodica_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Foodica_Theme_Plugin_Enhancements;
		}
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s is necessary to use some of this theme&rsquo;s features, including: ', 'foodica' ),
					'<strong>' . esc_html__( 'Jetpack plugin', 'foodica' ) . '</strong>' ),
	...
	function get_theme_dependencies() {
		...
		if ( current_theme_supports( 'site-logo' ) ) :
			$dependencies['logo'] = array(
				'name' => esc_html__( 'Site Logo', 'foodica' ),
				'slug' => 'site-logo',
				'url'  => '',
				'module' => 'none',
			);
		...
		if ( current_theme_supports( 'featured-content' ) ) :
			$dependencies['featured-content'] = array(
				'name' => esc_html__( 'Featured Content', 'foodica' ),
		...
		if ( current_theme_supports( 'jetpack-social-menu' ) ) :
			$dependencies['social-menu'] = array(
				'name' => esc_html__( 'Social Menu', 'foodica' ),
		...
		if ( current_theme_supports( 'jetpack-comic' ) ) :
			$dependencies['comics'] = array(
				'name' => esc_html__( 'Comics', 'foodica' ),
				'slug' => 'jetpack-comic',
				'url'  => '',
				'module' => 'custom-content-types',
			);
		...
		if ( current_theme_supports( 'jetpack-testimonial' ) ) :
			$dependencies['testimonials'] = array(
				'name' => esc_html__( 'Testimonials', 'foodica' ),
		...
		if ( current_theme_supports( 'jetpack-portfolio' ) ) :
			$dependencies['portfolios'] = array(
				'name' => esc_html__( 'Portfolios', 'foodica' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => esc_html__( 'no specific module needed', 'foodica' ),
			'custom-content-types' => esc_html__( 'Custom Content Types module', 'foodica' ),
		);
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'foodica_jetpack_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please activate %1$s. %2$s', 'foodica' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate', 'foodica' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please install %1$s. %2$s', 'foodica' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install', 'foodica' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				esc_html__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'foodica' ),
...
add_action( 'admin_head', array( 'Foodica_Theme_Plugin_Enhancements', 'init' ) );

function foodica_jetpack_enqueue_scripts() {
	...
	if ( is_admin() && get_user_meta( get_current_user_id(), 'foodica_jetpack_admin_notice', true ) !== 'dismissed' ) {
		...
		wp_localize_script( 'foodica_jetpack_admin_script', 'foodica_jetpack_admin', array(
			'foodica_jetpack_admin_nonce' => wp_create_nonce( 'foodica_jetpack_admin_nonce' ),
		));
	}
}
add_action( 'admin_enqueue_scripts', 'foodica_jetpack_enqueue_scripts' );
 ...
 */
function foodica_jetpack_dismiss_admin_notice() {
	// Verify the security nonce and die if it fails
	if ( ! isset( $_POST['foodica_jetpack_admin_nonce'] ) || ! wp_verify_nonce( $_POST['foodica_jetpack_admin_nonce'], 'foodica_jetpack_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'foodica' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'foodica_jetpack_admin_notice', 'dismissed' );
	// Send success message
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'foodica' )
	) );
}
add_action( 'wp_ajax_foodica_jetpack_admin_notice', 'foodica_jetpack_dismiss_admin_notice' );
