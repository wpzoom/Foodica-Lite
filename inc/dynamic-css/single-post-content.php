...
add_filter( 'foodica/dynamic_theme_css/selectors', 'foodica_selector_single_post_content' );

if ( ! function_exists( 'foodica_selector_single_post_content' ) ) {
	/**
	 ...
	 */
	function foodica_selector_single_post_content( $selectors ) {
		$selectors['typo-post-content']            = '.single .entry-content, .page .entry-content';
		$selectors['post-content-font-size-media'] = '@media screen and (min-width: 782px)';
		return $selectors;
	}
}

add_filter( 'foodica/dynamic_theme_css', 'foodica_dynamic_theme_css_single_post_content' );
 ...
 */
function foodica_dynamic_theme_css_single_post_content( $dynamic_css ) {
	

	$post_content_font_family    = foodica_get_font_stacks( foodica_get_theme_mod( 'post-content-font-family' ) );
	$post_content_font_size      = foodica_get_theme_mod( 'post-content-font-size' );
	$post_content_font_weight    = foodica_get_theme_mod( 'post-content-font-weight' );
	$post_content_text_transform = foodica_get_theme_mod( 'post-content-text-transform' );
	$post_content_line_height    = foodica_get_theme_mod( 'post-content-line-height' );
	$selectors   = apply_filters( 'foodica/dynamic_theme_css/selectors', array() );
	$selector    = foodica_get_prop( $selectors, 'typo-post-content' );
	$media_query = foodica_get_prop( $selectors, 'post-content-font-size-media' );
