...
if ( class_exists( 'WP_Customize_Control' ) ) {
	 ...
	 * Typography control.
	 */
	class Foodica_Customize_Typography_Control extends WP_Customize_Control {
		 ...
		 * @var string $type
		 */
		public $type = 'foodica-font-family';
		 ...
		 *
		 * @since 1.3.0
		 * @var string $foodica_inherit
		 */
		public $foodica_inherit = '';
		...
		public function __construct( $manager, $id, $args = array() ) {
			$this->foodica_inherit = __( 'Inherit', 'foodica' );
...
<select data-inherit="<?php echo esc_attr( $this->foodica_inherit ); ?>" <?php $this->link(); ?> class="{{ data.font_type }}" data-name="{{ data.id }}" data-value="{{data.value}}" <# if ( data.connect ) { #>data-connected-control={{ data.connect }}<# } #> <# if ( data.variant ) { #>data-connected-variant="{{data.variant}}"<# } #>>
</select>
