/* global jQuery, foodicaCustomControl, FoodicaFontFamilies */
...
( function ( $ ) {
	 ...
	 *
	 * @since 1.2.3
	 * @class FoodicaTypography
	 */
	const FoodicaTypography = {
		/**
		 ...
		 */
		_initFonts() {
			$( '.customize-control-foodica-font-family select' ).each(
				function () {
					if (
						'undefined' !== typeof foodicaCustomControl.customizer
					) {
						const fonts =
							foodicaCustomControl.customizer.settings
								.google_fonts;
			...
			$( '.customize-control-foodica-font-family select' ).each(
				FoodicaTypography._initFont
			);
			// Added select2 for all font family & font variant.
			$(
				'.customize-control-foodica-font-family select, .customize-control-foodica-font-variant select'
			).selectWoo();

			$( '.customize-control-foodica-font-variant select' ).on(
				'select2:unselecting',
				function ( e ) {
		...
		_initFont() {
			...
			if (
				...
				api( link ).bind( FoodicaTypography._fontSelectChange );
				FoodicaTypography._setFontWeightOptions.apply( api( link ), [
					true,
				] );
			}
			...
			if (
				...
				api( link ).bind( FoodicaTypography._fontSelectChange );
				FoodicaTypography._setFontVariantOptions.apply( api( link ), [
					true,
				] );
			}
		},
		...
		_fontSelectChange() {
			...
			FoodicaTypography._setFontWeightOptions.apply( this, [ false ] );

			if ( 'undefined' !== typeof variants ) {
				FoodicaTypography._setFontVariantOptions.apply( this, [
					false,
				] );
			}
		},
		...
		_cleanGoogleFonts( fontValue ) {
			...
			if (
				'undefined' !==
				typeof FoodicaFontFamilies.google[ googleFontValue ]
			) {
		...
		_getWeightObject( fontValue ) {
			...
			if ( fontValue === 'inherit' ) {
				...
				'undefined' !== typeof FoodicaFontFamilies.system[ fontValue ]
			) {
				weightObject = FoodicaFontFamilies.system[ fontValue ].weights;
			} else if (
				'undefined' !== typeof FoodicaFontFamilies.google[ fontValue ]
			) {
				weightObject = FoodicaFontFamilies.google[ fontValue ][ 0 ];
				weightObject = Object.keys( weightObject ).map( function ( k ) {
					return weightObject[ k ];
				} );
			} else if (
				'undefined' !== typeof FoodicaFontFamilies.custom[ fontValue ]
			) {
				weightObject = FoodicaFontFamilies.custom[ fontValue ].weights;
			}
		...
		_setFontWeightOptions( init ) {
			const fontSelect = api
				...
				weightSelect = api
					.control( weightKey )
					.container.find( 'select' ),
				currentWeightTitle = foodicaCustomControl.strings.inherit,
				inheritWeightObject = [ 'inherit' ],
				weightMap = foodicaCustomControl.font_weight;
			...
			fontValue = FoodicaTypography._cleanGoogleFonts( fontValue );
			weightObject = FoodicaTypography._getWeightObject( fontValue );
		...
		_setFontVariantOptions( init ) {
			...
			const fontSelect = api
				...
				variantSavedField = api
					.control( variants )
					.container.find( '.foodica-font-variant-hidden-value' ),
			...
			fontValue = FoodicaTypography._cleanGoogleFonts( fontValue );
			const weightObject = FoodicaTypography._getWeightObject(
				fontValue
			);
